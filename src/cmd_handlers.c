...
void handle_cd_cmd (char *argsv[], int argc) {
    ...
    int cd_code;  // Stores the return value of the chdir() function

    // Get the HOME environment variable that contains path to home directory
...
void handle_pwd_cmd(char *argsv[], int argc) {
    ...
    if (mode_arr[0] == 1) {
        ...
        // Close this descriptor because it is no longer used
        close(out);
    }
...
void handle_type_cmd (char *argsv[], int argc) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        // This loop checks if the current argument is a builtin command
