...
int is_redirecting (char *argsv[], int argc, int mode_arr[2]) {
    ...
    for (int i = argc - 1; i >= 1; i--) {
        if (strcmp(argsv[i], ">") == 0 || strcmp(argsv[i], "1>") == 0) {
            /* Redirect stdout to a file and overwrite file content */
            argsv[i] = NULL;
            mode_arr[0] = 1;
            mode_arr[1] = 0;
            return i + 1;
            ...
            /* Redirect stderr to a file and overwrite file content */
            argsv[i] = NULL;
            mode_arr[0] = 2;
            mode_arr[1] = 0;
            return i + 1;
            ...
            /* Redirect stdout to a file and append to file content */
            argsv[i] = NULL;
            mode_arr[0] = 1;
            mode_arr[1] = 1;
            return i + 1;
            ...
            /* Redirect stderr to a file and append to file content */
            argsv[i] = NULL;
            mode_arr[0] = 2;
            mode_arr[1] = 1;
            return i + 1;
...
char *locate_executable (const char *file_name) {
    ...
    */
    ...
    char *dir = strtok(path_copy, PATH_DELIMITER);  // Initiate parsing path_copy and get the first path.
    static char full_path[PATH_MAX];  // Stores the full absolute path into the executable that will be checked.

    bool file_found = false;
    while (dir != NULL) {
        // Print the absolute path to the executable into the full_path variable.
    ...
    free(path_copy);  // Free path_copy because strdup() returns a dynamically allocated pointer.
